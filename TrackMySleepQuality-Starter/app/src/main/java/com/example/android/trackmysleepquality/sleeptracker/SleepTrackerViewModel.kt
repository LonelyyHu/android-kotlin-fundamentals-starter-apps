...
class SleepTrackerViewModel(val database: SleepDatabaseDao, application: Application) : AndroidViewModel(application) {
    ...
    val nightsString = nights.map { nightsList ->
    ...
    val startButtonEnabled = tonight.map {
        it == null
    }

    val stopButtonEnabled = tonight.map {
        it != null
    }
    ...
    fun onStopTracking() {
        uiScope.launch {
            ...
            currentNight.endTimeMilli = System.currentTimeMillis()
            update(currentNight)
            _navigateToSleepQuality.value = currentNight
        }
    }
